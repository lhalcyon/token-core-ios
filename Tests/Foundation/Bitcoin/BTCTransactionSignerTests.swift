...
class BTCTransactionSignerTests: TestCase {
  ...
  func testSignTransactionMultipleUTXO() {
    do {
      ...
      let signedResult = try WalletManager.btcSignTransaction(wallet: wallet, to: "moLK3tBG86ifpDDTqAQzs4a9cUoNjVLRE3", amount: 750000000, fee: 502130, password: TestData.password, outputs: outputs, changeIdx: 53)
  ...
  func testInsufficientFunds() {
    do {
      ...
      XCTAssertThrowsError(try WalletManager.btcSignTransaction(wallet: wallet, to: "moLK3tBG86ifpDDTqAQzs4a9cUoNjVLRE3", amount: 410000000, fee: 502130, password: TestData.password, outputs: outputs, changeIdx: 53))
    } catch {
      XCTFail()
    }
  }
  ...
  func testSignSegWetTransaction() {
    do {
      ...
      let signedResult = try WalletManager.btcSignTransaction(wallet: wallet, to: "2N9wBy6f1KTUF5h2UUeqRdKnBT6oSMh4Whp", amount: 80000, fee: 10000, password: TestData.password, outputs: outputs, changeIdx: 0)
  ...
  func testSegWitSignDustOutput() {
    do {
      ...
      do {
        _ = try WalletManager.btcSignTransaction(wallet: wallet, to: "2N9wBy6f1KTUF5h2UUeqRdKnBT6oSMh4Whp", amount: 2000, fee: 10000, password: TestData.password, outputs: outputs, changeIdx: 0)
        XCTFail("Should throw amount_less_than_minimum")
      } catch {
        XCTAssertEqual(GenericError.amountLessThanMinimum.localizedDescription, error.localizedDescription)
      }
      
      let signedResult = try WalletManager.btcSignTransaction(wallet: wallet, to: "2N9wBy6f1KTUF5h2UUeqRdKnBT6oSMh4Whp", amount: (50000 + 50000 - 12000), fee: 10000, password: TestData.password, outputs: outputs, changeIdx: 0)
